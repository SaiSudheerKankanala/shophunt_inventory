...
<head>
    ...
    <style>
        ...
        .search-section { background: #e9ecef; padding: 15px; border-radius: 5px; margin-bottom: 20px; }
    </style>
</head>
<body>
    <div class="container">
    ...
    <script>
        // Use relative paths for Railway
        const API_BASE = '';
        ...
        async function deleteInventory(recordId) {
            ...
            try {
                const response = await fetch(API_BASE + '/api/inventory/delete/' + recordId, { method: 'DELETE' });
                const result = await response.json();
                if (response.ok) {
        ...
        async function postData(endpoint, data, responseElementId) {
            try {
                ...
                element.innerHTML = response.ok ? '✅ ' + result.message : '❌ Error: ' + result.error;
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO product_inventory (
                shop_name, shop_owner, shop_address, product_name, product_brand, 
                product_mrp, product_size, quantity, selling_price, manufacture_date, 
                expiry_date, stock_status, is_available
        ...
        return jsonify({'message': 'Product added successfully!', 'record_id': record_id}), 201
    except Exception as e:
...
@app.route('/health')
def health():
    return jsonify({'status': 'healthy', 'message': 'Shop Hunt API is running'})
